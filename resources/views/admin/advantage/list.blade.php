    ...
    <div class="col-md-12">
      <div class="panel panel-default">
        <div class="panel-heading">
          <h3 class="panel-title">优势列表</h3></div>
        <div class="panel-body">
          <div role="grid" id="example_wrapper" class="dataTables_wrapper form-inline no-footer">
          <form action="/advantage/list">
            <div class="row">
              <div class="col-xs-6">
              ...
              <tbody>
                    @foreach($advantage as $k=>$v)
                        <tr class="even" style="text-align: center;">
                            ...
                            <td>{{$v->title}}</td>
                            <td>{{$v->keyword}}</td>
                            <td><a href="/advantage/edit?id={{$v->id}}">修改</a> | <a href="/advantage/dele?id={{$v->id}}">删除</a></td>
                        </tr>
                    @endforeach
              </tbody>
            </table>
            <div class="row">
              ...
              <div class="col-xs-6">
                <div class="dataTables_paginate paging_simple_numbers" id="example_paginate">
                   <ul class="pagination">
                      {!! $advantage->appends($request->only(['list_length','title']))->render() !!} 
                    </ul>
                </div>
              </div>
            </div>
